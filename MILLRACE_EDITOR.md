...
Millrace Editor loads `MILLRACE_EDITOR.md` from the workspace root as agent memory (similar to AGENTS.md / CLAUDE.md). This file is also the project's living architecture doc — read it before making changes.

## Project

**Millrace Editor** — Terax-derived AI-native development workbench with governed Millrace workflows. Tauri 2 + Rust (`portable-pty`) backend, React 19 + TypeScript + xterm.js (webgl) client, BYOK AI via Vercel AI SDK v6.

- Bundle id: `dev.millrace.editor`
...
- `secrets::secrets_*` — OS keychain via the `keyring` crate. Service constant `millrace-editor-ai`. Linux uses a file-based fallback gated behind `#[cfg(target_os = "linux")]`.
...
- **Key storage**: OS keychain via `keyring` (Rust). Frontend reads/writes through `secrets_*` commands. Service `KEYRING_SERVICE = "millrace-editor-ai"`. Never persist keys to disk, settings store, or `localStorage`.
...
- Auto-updater is disabled until Millrace Editor has its own release channel.
