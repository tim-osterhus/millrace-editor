...
const INIT_PROMPT = `Scan this workspace and produce MILLRACE_EDITOR.md at the workspace root with:
...
Use grep/glob/list_directory/read_file to explore. Cap MILLRACE_EDITOR.md under 200 lines. Use write_file to create it (will go through normal approval).`;
...
export const MILLRACE_EDITOR_CMD_RE =
  /^<millrace-editor-command\s+name="([a-z0-9-]+)"(?:\s+state="([a-z]+)")?\s*\/>(?:\n+|$)/;

export function wrapWithCommandMarker(prompt: string, name: string): string {
  return `<millrace-editor-command name="${name}" />\n\n${prompt}`;
}
